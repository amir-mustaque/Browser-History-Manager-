...
public:
  ...
  void visitURL(const string& url) {
    // If the current URL is not at the end, it means we're in the middle of the history
    // Only clear the forward stack when we're at the latest URL
    if (current == --historyList.end()) {
        forwardStack = stack<string>(); // Clear forward history only if we're at the most recent URL
    }
    historyList.push_back(url);
    current = --historyList.end(); // Update current to the latest URL
    backStack.push(url);           // Push the new URL onto the back stack
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 9:
                ...
                // Input validation for tab choice
                if (!(cin >> tabChoice) || tabChoice < 1) {
                    cout << "Invalid input. Please enter a valid tab number." << endl;
                    cin.clear(); // Clear the error state
                    cin.ignore(numeric_limits<streamsize>::max(), '\n'); // Ignore the rest of the input
                    continue;
                }
